...
class ICLAttentionC(nn.Module):
    def __init__(self, config):
        ...
        self.W_q = nn.Parameter(torch.zeros(config.n_heads, config.d_embed, config.d_hidden), requires_grad=True)
        self.W_k = nn.Parameter(torch.zeros(config.n_heads, config.d_embed, config.d_hidden), requires_grad=True)
...
class ICLAttentionA(nn.Module):
    def __init__(self, config):
        super().__init__()
        
        self.config = config
        
        self.W_qk = nn.Parameter(torch.zeros(config.n_heads, config.d_embed, config.d_hidden), requires_grad=True)
        self.W_o = nn.Parameter(torch.zeros(config.n_heads * config.d_embed, config.d_embed), requires_grad=True)
    ...
    def forward(self, q, k, v):
        ...
        q = torch.einsum('bsz,hzd->bhsd', q, self.W_qk) # (B, H, S, D)
        k = torch.einsum('bsz,hzd->bhsd', k, self.W_qk) # (B, H, S, D)
